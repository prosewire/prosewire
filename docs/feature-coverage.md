...
| SMTP reliability | Scoped transport shutdown waits for active sends; DNS/connection/greeting timeouts are ten seconds and socket inactivity timeout is thirty seconds. Only transient SMTP failures retry, for at most six attempts; permanent and exhausted failures are logged with the outbox ID |
| Workflow history retention | Daily scans clear terminal email workflow messages and replies after thirty days, scrub the delivery payload, remove Redis deduplication identities when the queue is idle, preserve outbox deduplication records, and resume interrupted cleanup; active executions remain intact |
