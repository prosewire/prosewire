...
| SMTP reliability | Scoped transport shutdown waits for active sends; connection/greeting timeouts are ten seconds and socket inactivity timeout is thirty seconds. Only transient SMTP failures retry, for at most six attempts; permanent and exhausted failures are logged with the outbox ID |
