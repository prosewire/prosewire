...
description: Runtime, authentication, media, email, bootstrap, worker, and documentation-site settings for Prosewire.
section: Reference
order: 8
---
...
| Variable | Required | Purpose |
| --- | --- | --- |
| `PROSEWIRE_MEDIA_ENDPOINT` | Cloud; optional self-hosted | S3-compatible API endpoint |
| `PROSEWIRE_MEDIA_REGION` | No | Object-storage region; defaults to `auto` |
| `PROSEWIRE_MEDIA_BUCKET` | With endpoint | Private bucket for uploads and processed variants |
| `PROSEWIRE_MEDIA_ACCESS_KEY_ID` | With endpoint | Object-storage access-key identifier |
| `PROSEWIRE_MEDIA_SECRET_ACCESS_KEY` | With endpoint | Object-storage secret key |
| `PROSEWIRE_MEDIA_PUBLIC_URL` | With endpoint | CDN origin used for processed asset URLs |
| `PROSEWIRE_MEDIA_FORCE_PATH_STYLE` | No | Enables path-style bucket addressing; defaults to `false` |
| `PROSEWIRE_MEDIA_MAX_UPLOAD_BYTES` | No | Maximum reservation size from 1 byte through 100 MiB; defaults to 20 MiB |
| `PROSEWIRE_MEDIA_UPLOAD_URL_EXPIRES_SECONDS` | No | Signed upload lifetime from 60 through 3600 seconds; defaults to 600 |

Set all five endpoint, bucket, credential, and public URL values together. Cloud mode rejects a missing media configuration. Self-hosted mode can omit it and continue using external cover-image URLs.

The media bucket must stay private. Put a CDN in front of it and configure the CDN to serve only processed keys under `publications/`. Deny the temporary `_uploads/` prefix. Processed filenames contain a checksum, so the CDN may cache them; Prosewire records the resulting CDN URL with each variant.

Browser uploads use a signed `PUT` request. The bucket CORS policy must allow the exact Prosewire dashboard origin, the `PUT` method, and the `Content-Type` request header. Do not use a wildcard origin for a credentialed production dashboard. The application checks the reserved byte count against the object metadata, reads the object back, detects its MIME type from the image, rejects animated or multi-page inputs, and produces sanitized original, 1600-pixel, and 480-pixel variants.

Each publication starts with a 1 GiB storage quota recorded in Postgres. Pending reservations and ready variants count toward the quota. Operators can change `blog.media_storage_quota_bytes` through a controlled database operation until a quota administration surface is added.

Media storage credentials must allow reading, writing, deleting, and listing objects in the configured bucket. Listing the `publications/` prefix lets recovery remove variants left by a terminated web process. Processing times out after five minutes; if a process is killed before cleanup, retry completion or remove the asset after its 15-minute processing lease expires. Both operations release its reserved quota and remove abandoned variants. Storage cleanup failures are logged and removal can be retried.
...
| `SMTP_URL` | Required at production worker startup | SMTP or SMTPS connection URL |
...
The worker runs awaited scans for scheduled publishing, raw-view retention, and outbox draining, and starts durable workflows for email delivery. Postgres `LISTEN/NOTIFY` normally drains the outbox immediately. Scans log failures and retry on the next cycle; retention also retries three times at one-minute intervals. Empty scans create no persisted workflow execution. A 30-second scan catches notifications missed while the listener reconnects, and expired leases resume safely after a restart. The email workflow waits for an Effect `DurableQueue` worker in Redis. Transient SMTP failures use exponential retry with a thirty-second cap and finish as a typed workflow failure after six attempts. Permanent recipient, authentication, and configuration failures stop immediately. A terminal error log includes the outbox ID; after correcting the cause, issue a new invitation to create a fresh delivery intent. SMTP DNS, connection, and greeting timeouts are ten seconds; socket inactivity timeout is thirty seconds. Shutdown waits for an active send to settle before releasing its queue lease and closing the transport. These are per-phase and inactivity limits, not an absolute shutdown deadline: an SMTP peer that keeps making progress can extend the send. SMTP delivery remains at-least-once when a process dies after the server accepts a message.
...
- Back up Postgres, Redis, and the media buckets, then test a coordinated restore before launch

### Email execution retention

The worker checks email executions daily. Once a dispatched intent is older than
thirty days and its workflow has a terminal result, it clears that workflow's
persisted messages/replies and scrubs the recipient, subject, body, and error
from the outbox row. The outbox ID and dispatch marker remain as a deduplication
record. Running, suspended, unknown, and recent executions are preserved.
Cleanup also removes the matching Redis queue identity. It preserves queued, pending, locked, and failed queue identities. Legacy identities defer cleanup while the queue has waiting work. Cleanup records its intent before clearing workflow storage, so a restart can
finish cleanup even when the workflow result has already been removed.

Migration `0016_cold_darkhawk` adds two nullable cleanup timestamps without
rewriting existing payloads. Run the normal one-shot migration before starting
the new worker. Back up PostgreSQL before upgrading; rolling back the worker
requires no schema rollback because old workers ignore the new columns. To
recover historical delivery payloads after retention, restore the matching
PostgreSQL backup to a separate database for inspection. Do not clear dispatch
markers or restore only workflow tables into a live worker database, because
that can replay previously dispatched email.

Effect RC.115 stores queue identities in Redis sorted sets. At startup the worker
converts legacy sets in restartable batches, retaining the original set until
the final atomic replacement. Stop the old worker and back up Redis before
upgrading; run exactly one worker during conversion. Pending payloads and locks
are preserved. To roll back to RC.112, stop the worker and restore the matching
Redis and PostgreSQL backups; an old worker cannot read the new sorted sets.
