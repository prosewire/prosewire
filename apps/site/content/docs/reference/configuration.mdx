...
| `REDIS_URL` | Worker | Redis connection used by workers for the Effect persisted email queue |
...
Use the same `DATABASE_URL` for the migration, web, and worker processes. Each worker replica reserves one dedicated Postgres connection for email outbox notifications. Every worker replica must use the same `REDIS_URL`; the web process writes email intents to Postgres and does not connect to Redis. In Compose, choose a URL-safe `POSTGRES_PASSWORD` because it is interpolated into a connection URL.

The Redis account must permit `SCRIPT LOAD`, `EVALSHA`, publish, subscribe, list, set, hash, string, and expiry commands. Use AOF persistence, replication, backups, and `maxmemory-policy noeviction` in production. Postgres retains the original email intent; Redis owns delivery processing, retries, and terminal failures after dispatch. Back up both systems because losing Redis after Postgres marks an intent dispatched can still lose an undelivered queue job.
