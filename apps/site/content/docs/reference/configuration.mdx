...
| `REDIS_URL` | Worker | Redis connection used by the Effect persisted email queue |
...
Use the same `DATABASE_URL` for the migration, web, and worker processes. Every worker replica must use the same `REDIS_URL`. In Compose, choose a URL-safe `POSTGRES_PASSWORD` because it is interpolated into a connection URL.

The Redis account must permit `SCRIPT LOAD`, `EVALSHA`, list, hash, string, and expiry commands. Use AOF persistence, replication, and `maxmemory-policy noeviction` in production. Prosewire keeps the email outbox in Postgres and republishes a stale dispatch after five minutes, so Redis loss delays an email instead of removing its source record.
