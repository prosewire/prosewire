...
| `PROSEWIRE_MEDIA_BUCKET` | With endpoint | Private primary bucket for uploads and processed variants |
...
| `PROSEWIRE_MEDIA_BACKUP_BUCKET` | No | Second bucket that receives processed media copies |
...
The primary bucket must stay private. Put a CDN in front of it and configure the CDN to serve only processed keys under `publications/`. Deny the temporary `_uploads/` prefix. Processed filenames contain a checksum, so the CDN may cache them; Prosewire records the resulting CDN URL with each variant.
...
When a backup bucket is configured, upload completion copies every processed variant and records the completed backup time. A failed backup does not mark an upload ready. Manual retry is available in the dashboard and management clients. The backup bucket uses the same S3 endpoint and credentials, should have versioning and retention enabled, and should use a separate failure domain where the provider supports one.
