...
- A deployment-specific `NEXT_DEPLOYMENT_ID` for the image build
- S3-compatible `PROSEWIRE_MEDIA_*` settings if editors need managed uploads
...
Migration `0015_freezing_norman_osborn.sql` adapts accounts to Better Auth 1.7.5. It retains historical `issuer` values, makes the field optional, and replaces issuer-based uniqueness with `(provider_id, account_id)` uniqueness. Before upgrading, stop authentication writes, back up Postgres, and check for collisions:
...
The query must return no rows. If it finds collisions, keep the old release running and review the account owners and provider configuration. Do not merge different users or delete their accounts automatically. The migration creates the new unique index first and refuses collisions before relaxing the old constraints. Resolve each identity using trusted provider records, then rerun the migration. Deploy web and worker together after migration. To return to Better Auth 1.7.0 through 1.7.2 after accepting new accounts, restore the pre-upgrade database backup and previous application together; new accounts can lack an issuer, so reverting the application alone is unsafe.
...
Back up Postgres with managed snapshots or `pg_dump`. Back up the Redis volume separately because delivery processing, retries, and failed email jobs live there after outbox dispatch. If media storage is configured, snapshot or replicate the media bucket and record the same recovery point as the database backup. A Compose-hosted Postgres example is:

```shell
docker compose exec -T postgres pg_dump -U prosewire -d prosewire --format=custom > prosewire.dump
```

Store backups outside the application host, encrypt them when appropriate, and define a retention policy. Test restoration into a separate database and object-storage namespace. Verify posts, authors, redirects, memberships, audit entries, media checksums, exports, and public visibility before calling the backup usable.
...
Portable JSON, CSV, and media ZIP exports improve content portability but are not substitutes for coordinated database and object-storage backups because Prosewire does not currently provide a full import workflow.

Migration `0014_yummy_wildside.sql` only adds tables, columns, indexes, and constraints. Existing external cover URLs remain unchanged and their managed-asset identifier stays null. Before rollback, stop writes and take a database backup. An older application ignores the new media tables and column, but cannot understand managed cover references. Restore the database and media bucket together if the migration or upload rollout must be reversed. Do not drop the new tables until every post has `cover_image_asset_id` cleared and a replacement external URL has been verified.
...
- Keep the media bucket private, restrict CDN paths to processed objects, and test signed upload CORS from the dashboard origin
...
- Verify a coordinated database, Redis, and media restore plus every export format before launch
