...
The web and worker wait for the migration service and Redis before starting. The web enqueues invitation email jobs; workers consume them. Production startup does not create the development administrator, API key, publication, or sample posts.
...
Back up Postgres with managed snapshots or `pg_dump`. Back up the Redis volume separately because queued and failed email jobs exist only in Redis. A Compose-hosted Postgres example is:
...
- Run at least one worker replica; all web producers and worker consumers must use the same Redis queue
