...
Migration `0014_yummy_wildside.sql` only adds tables, columns, indexes, and constraints. Existing external cover URLs remain unchanged and their managed-asset identifier stays null. Before rollback, stop writes and take a database backup. An older application ignores the new media tables and column, but cannot understand managed cover references. Restore the database and primary media bucket together if the migration or upload rollout must be reversed. Do not drop the new tables until every post has `cover_image_asset_id` cleared and a replacement external URL has been verified.
