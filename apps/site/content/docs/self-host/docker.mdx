...
The web and worker wait for the migration service; the worker also waits for Redis. The web commits invitation email intents to Postgres. Workers lease those intents, dispatch them to Redis, and consume the queue. Production startup does not create the development administrator, API key, publication, or sample posts.
...
The `email_delivery_outbox` table is safe to leave in place when rolling the application back, but an older web release does not create outbox intents. If Redis is lost and restored empty, stop workers, restore Postgres to the matching recovery point, reset affected outbox rows to pending only after confirming their queue jobs are absent, then restart one worker and verify delivery before scaling out.
...
- Run at least one worker replica; every worker dispatcher and consumer must use the same Redis queue
