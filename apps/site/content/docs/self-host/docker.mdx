...
The web and worker wait for the migration service to complete, and the worker also waits for Redis. Production startup does not create the development administrator, API key, publication, or sample posts.
...
- Run at least one worker replica; multiple replicas coordinate through Postgres and consume the same Redis queue
