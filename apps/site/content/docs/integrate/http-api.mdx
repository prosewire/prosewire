...
Create and update operations render and sanitize Markdown before persistence. Restore applies content that Prosewire rendered and stored in an earlier revision. Every mutation creates an audit record. Expect 400 for invalid input, 401 for a missing or invalid key, 403 for an insufficient scope or publication boundary, 404 for a missing post or revision, and 500 when the service cannot complete the operation.
