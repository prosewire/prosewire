...
The web app and worker run against your Postgres database. Invitation creation writes its email to a Postgres outbox in the same transaction, then workers dispatch the outbox identifier to an Effect `PersistedQueue` in Redis. Any worker replica can consume the job. A Postgres lease prevents duplicate queue messages from sending concurrently, and stale rows are dispatched again after a worker or Redis failure.
