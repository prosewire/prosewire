...
The web runtime builds the publishing and workspace repository Layers once. Entrypoints call these services directly, where transactional authorization and publishing policy live. Command schemas come from their owning modules.
...
The worker keeps a dedicated Postgres `LISTEN` connection. A notification drains the email outbox as soon as the invitation transaction commits. A 30-second scan runs the same drain for notifications missed during a disconnect or restart. The outbox drain leases rows with `FOR UPDATE SKIP LOCKED` and starts one idempotent email workflow per outbox ID. It marks the row dispatched only after the workflow engine has accepted that execution.
...
Scheduled publishing and analytics retention run as supervised, awaited Effect scans. A failed scan is logged and remains retryable; no workflow execution is persisted for an empty poll. Analytics retention retries three times at one-minute intervals before the next daily cycle. Their source records remain in Postgres, so a restart can safely run another scan without creating a second content state. Content remains exportable, and the built-in public reader remains readable without client-side JavaScript.
