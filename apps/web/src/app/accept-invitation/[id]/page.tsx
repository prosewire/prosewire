      ...
      <main className="flex min-h-screen items-center justify-center bg-[#f4f3ed] px-5">
        ...
        <div className="card w-full max-w-[480px] p-7">
          ...
          <p className="mt-3 text-sm leading-6 text-[#69757a]">
            Use {details.invitation.email} so Prosewire can match this
            invitation.
          </p>
          <div className="mt-7 grid gap-3">
            <Link
              href={`/sign-in?returnTo=${encodeURIComponent(returnTo)}`}
              className="grid h-12 place-items-center rounded-xl bg-[#172329] text-sm font-bold text-white"
            >
              Sign in
            </Link>
            <Link
              href={`/sign-up?returnTo=${encodeURIComponent(returnTo)}`}
              className="grid h-12 place-items-center rounded-xl border border-[#d9dbd5] bg-white text-sm font-bold"
            >
              Create an account
            </Link>
          </div>
