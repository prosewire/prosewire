...
export default async function PublicBlogPage({ params, searchParams }: { params: Promise<{ blog: string }>; searchParams: Promise<{ q?: string; category?: string; page?: string }> }) {
  ...
  const featured = page === 1 ? (posts.find((post) => post.featured) ?? posts[0]) : undefined;
    ...
    <main style={blogStyle(blog.accentColor)} className="min-h-screen bg-[#f8f7f2] text-[#172329]">
      ...
      <section className="border-b border-black/10 bg-[#efeee7]">
        <div className="mx-auto max-w-6xl px-5 py-16 sm:py-20">
          <p className="text-xs font-bold uppercase tracking-[.18em] text-[var(--blog-accent)]">Independent publishing</p>
          <h1 className="display-font mt-5 max-w-4xl text-5xl leading-[1.02] sm:text-7xl">{blog.name}</h1>
      ...
      <section className="mx-auto max-w-6xl px-5 pb-20"><div className="flex items-end justify-between border-b border-black/10 pb-4"><div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-[var(--blog-accent)]">Latest</p><h2 className="display-font mt-2 text-3xl">More from {blog.name}</h2></div></div>{rest.length ? <div className="grid gap-x-8 gap-y-12 pt-8 md:grid-cols-2 lg:grid-cols-3">{rest.map((post, index) => <article key={post.id} className="group"><Link href={`/b/${blog.slug}/${post.slug}`}><div className={`paper-grid relative aspect-[1.55] overflow-hidden rounded-2xl border border-black/10 ${['bg-[#dfe8de]', 'bg-[#ece0d6]', 'bg-[#dfe4e8]'][index % 3]}`}><span className="absolute left-5 top-5 font-mono text-[10px] text-[#657077]">{String((page - 1) * pageSize + index + 1).padStart(2, '0')}</span><span className="absolute bottom-5 right-5 grid size-10 place-items-center rounded-full bg-white shadow-sm transition group-hover:translate-x-1"><ArrowRight className="size-4" /></span></div><p className="mt-5 text-[10px] font-bold uppercase tracking-[.14em] text-[var(--blog-accent)]">{post.categories[0]?.category.name ?? 'Article'}</p><h3 className="display-font mt-2 text-2xl leading-tight">{post.title}</h3><p className="mt-3 line-clamp-3 text-sm leading-6 text-[#687279]">{post.excerpt}</p><p className="mt-4 text-[11px] text-[#8a9397]">{post.author.name} · {post.publishedAt?.toLocaleDateString("en", { month: "short", day: "numeric" })}</p></Link></article>)}</div> : <div className="py-16 text-center"><p className="text-sm font-semibold">No published stories match this search.</p><Link href={`/b/${blog.slug}`} className="mt-3 inline-block text-xs font-semibold text-[var(--blog-accent)]">Clear filters</Link></div>}<nav aria-label="Post pages" className="mt-10 flex justify-between">{page > 1 ? <Link href={pageHref(page - 1)} className="text-sm font-semibold text-[var(--blog-accent)]">← Newer posts</Link> : <span />}{hasMore ? <Link href={pageHref(page + 1)} className="text-sm font-semibold text-[var(--blog-accent)]">Older posts →</Link> : null}</nav></section>
      <footer className="border-t border-black/10"><div className="mx-auto flex max-w-6xl flex-col gap-4 px-5 py-8 text-xs text-[#7b8589] sm:flex-row sm:items-center sm:justify-between"><span className="display-font text-base font-bold text-[#172329]">{blog.name}</span><span>Published with Prosewire · <Link href={`/b/${blog.slug}/rss.xml`}>RSS</Link> · <Link href={`/b/${blog.slug}/sitemap.xml`}>Sitemap</Link></span></div></footer>
    </main>
  );
}
