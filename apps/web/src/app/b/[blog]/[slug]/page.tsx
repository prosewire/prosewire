...
export default async function PublicPostPage({
    ...
    .filter(
      (item) =>
        ...
        item.categories.some((entry) =>
          post.categories.some((own) => own.categoryId === entry.categoryId),
        ),
    )
    .slice(0, 2);
  ...
  const canonical =
    post.canonicalUrl ??
    (blog.publicUrl
      ? `${blog.publicUrl}/${post.slug}`
      : `${publicUrl}/b/${blog.slug}/${post.slug}`);
  const jsonLd = {
  ...
  return (
    ...
    >
      ...
      <article>
        <header className="border-b border-black/10 bg-[#efeee7]">
          <div className="mx-auto max-w-4xl px-5 py-14 sm:py-20">
            <Link
              href={`/b/${blog.slug}`}
              className="inline-flex items-center gap-2 text-xs font-semibold text-[#69757a]"
            >
              <ArrowLeft className="size-3.5" />
              All stories
            </Link>
            <p className="mt-10 text-[10px] font-bold uppercase tracking-[.18em] text-[var(--blog-accent)]">
              {post.categories[0]?.category.name ?? "Article"}
            </p>
            <h1 className="display-font mt-4 text-5xl leading-[1.03] sm:text-7xl">
              {post.title}
            </h1>
            <p className="mt-6 max-w-3xl text-lg leading-8 text-[#647076]">
              {post.excerpt}
            </p>
            <div className="mt-8 flex flex-wrap items-center gap-4 text-xs text-[#778186]">
              <Link
                href={`/b/${blog.slug}/authors/${post.author.slug}`}
                className="font-semibold text-[#172329]"
              >
                {post.author.name}
              </Link>
              ...
              <span className="flex items-center gap-1.5">
                ...
                {readingMinutes(post.contentMarkdown)} min read
              </span>
            </div>
          </div>
        </header>
        <div className="mx-auto grid max-w-6xl gap-12 px-5 py-12 lg:grid-cols-[180px_minmax(0,760px)] lg:justify-center lg:py-16">
          <aside className="hidden lg:block">
            <div className="sticky top-8">
              <p className="text-[10px] font-bold uppercase tracking-[.15em] text-[#9aa1a4]">
                On this page
              </p>
              <nav className="mt-4 space-y-2.5">
                {headings.map((heading) => (
                  <a
                    key={heading.id}
                    href={`#${heading.id}`}
                    className={`block text-[11px] leading-4 text-[#69757a] hover:text-[var(--blog-accent)] ${heading.level === 3 ? "pl-3" : "font-medium"}`}
                  >
                    {heading.label}
                  </a>
                ))}
          ...
          <div>
            ...
            />
            <div className="mt-14 rounded-2xl border border-black/10 bg-[#efeee7] p-6">
              <p className="text-[10px] font-bold uppercase tracking-[.15em] text-[var(--blog-accent)]">
                About the author
              </p>
              <div className="mt-4 flex gap-4">
                <div className="grid size-12 shrink-0 place-items-center rounded-full bg-[#172329] text-sm font-bold text-white">
                  {post.author.name
                    .split(" ")
                    .map((part) => part[0])
                    .join("")
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs font-medium text-[#ef6848]">
                    {post.author.jobTitle}
                  </p>
                  <p className="mt-3 text-sm leading-6 text-[#687279]">
                    {post.author.bio}
                  </p>
                  <p className="mt-2 text-[10px] text-[#8a9397]">
      ...
      {related.length ? (
        <section className="border-t border-black/10 bg-[#efeee7]">
          <div className="mx-auto max-w-6xl px-5 py-14">
            <p className="text-[10px] font-bold uppercase tracking-[.16em] text-[var(--blog-accent)]">
              Keep reading
            </p>
            ...
            <div className="mt-7 grid gap-4 md:grid-cols-2">
              {related.map((item) => (
                <Link
                  key={item.id}
                  href={`/b/${blog.slug}/${item.slug}`}
                  className="group rounded-2xl border border-black/10 bg-white p-6"
                >
                  <h3 className="display-font text-2xl leading-tight">
                    {item.title}
                  </h3>
                  <p className="mt-3 text-sm leading-6 text-[#687279]">
                    {item.excerpt}
                  </p>
                  <span className="mt-5 inline-flex items-center gap-2 text-xs font-semibold text-[var(--blog-accent)]">
                    Read story{" "}
