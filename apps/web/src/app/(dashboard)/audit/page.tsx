import { ClockCounterClockwise, ShieldCheck } from "@phosphor-icons/react/ssr";
      ...
      <section className="card mt-7 overflow-hidden">
        <div className="flex items-center justify-between border-b border-[#e2e3de] px-5 py-4">
          <div className="flex items-center gap-2">
            <ClockCounterClockwise className="size-4 text-[#ef6848]" />
            <h2 className="text-sm font-semibold">Recent events</h2>
          </div>
          <span className="text-xs text-[#8a9397]">
            Latest {entries.length} of 100
          </span>
        </div>
          ...
          {entries.map((entry) => (
            ...
            >
              <div className="flex min-w-0 items-start gap-3">
                <span className="mt-0.5 grid size-8 shrink-0 place-items-center rounded-lg bg-[#f0f8f3] text-[#1f6e52]">
