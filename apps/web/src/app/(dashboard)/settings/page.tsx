...
import { createPublication, updateBlogSettings, updateWorkspace } from "@/server/actions";
import { loadDashboardSettings } from "@/server/page-entrypoints";
import { dashboardData } from "../dashboard-result";

export const metadata = { title: "Settings" };

export default async function SettingsPage({ searchParams }: { searchParams: Promise<{ saved?: string; workspaceSaved?: string; error?: string }> }) {
  ...
  return (
    <main className="mx-auto max-w-[1000px] px-4 py-6 sm:px-7 lg:px-9 lg:py-8">
      ...
      {query.workspaceSaved === "1" ? <div className="mt-5 flex items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-xs font-semibold text-emerald-700"><CheckCircle2 className="size-4" />Workspace settings saved.</div> : null}
      ...
      {hasPermission(context.role, "workspace:update") ? <form action={updateWorkspace} className="card mt-6 flex flex-col gap-4 p-5 sm:flex-row sm:items-end"><input type="hidden" name="organizationId" value={context.workspace.id} /><label className="flex-1 text-xs font-semibold">Workspace name<input name="name" defaultValue={context.workspace.name} className="mt-2 h-10 w-full rounded-xl border border-[#d9dbd5] px-3 text-sm font-normal outline-none focus:border-[#ef6848]" /></label><button className="h-10 rounded-xl border border-[#d9dbd5] bg-white px-4 text-sm font-semibold">Save workspace</button></form> : null}
      {canCreatePublication ? <form action={createPublication} className="card mt-4 grid gap-4 p-5 sm:grid-cols-[1fr_1fr_auto] sm:items-end"><input type="hidden" name="organizationId" value={context.workspace.id} /><label className="text-xs font-semibold">New publication name<input name="name" required placeholder="Company journal" className="mt-2 h-10 w-full rounded-xl border border-[#d9dbd5] px-3 text-sm font-normal outline-none focus:border-[#ef6848]" /></label><label className="text-xs font-semibold">Public slug<input name="slug" placeholder="company-journal" className="mt-2 h-10 w-full rounded-xl border border-[#d9dbd5] px-3 text-sm font-normal outline-none focus:border-[#ef6848]" /></label><button className="h-10 rounded-xl bg-[#172329] px-4 text-sm font-semibold text-white">Create publication</button></form> : null}
      <form action={updateBlogSettings} className="mt-6 space-y-4">
        <input type="hidden" name="id" value={blog.id} />
        <fieldset disabled={!canUpdatePublication} className="space-y-4 disabled:opacity-75">
        ...
        <section className="card flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><span className="grid size-9 place-items-center rounded-xl bg-[#eef0ec]"><Database className="size-4 text-[#657077]" /></span><div><h2 className="text-sm font-semibold">Data ownership</h2><p className="mt-1 text-xs leading-5 text-[#7b8589]">Postgres is the source of truth. CSV exports are available from the posts screen.</p></div></div>{canUpdatePublication ? <button className="h-10 rounded-xl bg-[#172329] px-5 text-sm font-semibold text-white shadow-sm">Save settings</button> : <p className="text-xs font-semibold text-[#7b8589]">Read-only access</p>}</section>
        </fieldset>
      </form>
    </main>
  );
