  ...
  return (
    <main className="mx-auto max-w-[1400px] px-4 py-6 sm:px-7 lg:px-9 lg:py-8">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <Link href="/posts/new" className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-[#ef6848] px-4 text-sm font-semibold text-white shadow-sm"><Plus className="size-4" />New post</Link>
      </header>
      ...
      <form action={bulkArchivePosts} className="card mt-4 overflow-hidden">
        ...
        <div className="flex items-center justify-between border-b border-[#e2e3de] bg-[#fbfbf8] px-4 py-3"><p className="text-xs font-semibold text-[#6d777c]">{posts.length} posts</p><button className="inline-flex items-center gap-1.5 rounded-lg border border-[#d9dbd5] bg-white px-2.5 py-1.5 text-[11px] font-semibold text-[#687279]"><Archive className="size-3" />Archive selected</button></div>
            ...
            <tbody className="divide-y divide-[#ecece8]">
              {posts.map((post) => (
                <tr key={post.id} className="bg-white transition hover:bg-[#fafaf7]">
                  <td className="px-4 py-4"><input aria-label={`Select ${post.title}`} type="checkbox" name="postId" value={post.id} className="size-4 accent-[#ef6848]" /></td>
                  <td className="max-w-lg px-2 py-4"><Link href={`/posts/${post.id}/edit`} className="block"><p className="truncate text-sm font-semibold">{post.title}</p><p className="mt-1 truncate text-[11px] text-[#8a9397]">/{post.slug}</p></Link></td>
