...
interface PublicList {
  readonly posts: ReadonlyArray<{ readonly title: string; readonly slug: string }>;
}
...
async function signIn(page: Page, email: string): Promise<void> {
  await page.goto("/sign-in?returnTo=/posts");
...
function toolJson(result: unknown): unknown {
  ...
  const content = (result as { readonly content: ReadonlyArray<unknown> }).content;
  const block = content[0] as ToolText | undefined;
  expect(block?.type).toBe("text");
  return JSON.parse(block?.text ?? "null") as unknown;
}

test.describe.serial("Postgres-backed cross-surface content matrix", () => {
  test("public raw, rendered, browser, embed, RSS, and sitemap expose only due published content", async ({
    ...
    for (const slug of [
      ...
      expect(
        (await request.get(`/api/public/${acceptance.blog.slug}/posts/${slug}`)).status(),
      ).toBe(404);
    }
    ...
    const sitemap = await request.get(`/b/${acceptance.blog.slug}/sitemap.xml`);
    ...
    await expect(page.getByRole("heading", { name: "Acceptance Published" })).toBeVisible();
    expect(
    ...
    await expect(page.locator("[data-prosewire] article.pw-post")).toBeVisible();
  });
  ...
  test("typed SDK enforces tenancy and drives draft through scheduled, published, archived, and restored", async ({
    ...
    expect((await otherTenant.posts.list({ page: 1, pageSize: 100 })).items)
      .toHaveLength(1);
    ...
    await expect(
      client.posts.create({ ...createInput, blogId: acceptance.otherBlog.id }),
    ).rejects.toThrow();
    ...
    expect(scheduled).toMatchObject({ status: "scheduled", scheduledAt: expect.any(String) });

    const published = await client.posts.update({
      params: { id: draft.id },
      body: { status: "published", slug: "lifecycle-published" },
    });
    expect(published).toMatchObject({ status: "published", slug: "lifecycle-published" });
    ...
    await expect(client.posts.archive({ params: { id: draft.id } })).resolves.toEqual({ ok: true });
    expect(
      (
        await fetch(
    ...
    await expect(page.getByRole("heading", { name: "Lifecycle Acceptance" })).toBeVisible();
  });
  ...
  test("CLI reads public content and creates a private draft through the production API", async () => {
    ...
    try {
      ...
      const post = JSON.parse(created.stdout) as { id: string; status: string };
  ...
  test("MCP exposes real SDK reads and archives a Postgres-backed post", async () => {
    ...
    try {
      ...
      expect(posts.items.map((post) => post.title)).toContain("CLI Acceptance Draft");
  ...
  test("dashboard browser flow honors tenant membership and owner/viewer roles", async ({
    ...
    try {
      const owner = await withSignedInPage(ownerContext, acceptance.owner.email);
      await expect(owner.getByRole("link", { name: "New post" })).toBeVisible();
      await expect(owner.getByRole("button", { name: "Archive selected" })).toBeVisible();
      await expect(owner.getByText("Acceptance Published", { exact: true })).toBeVisible();
      await expect(owner.getByText("Other Tenant Secret", { exact: true })).toHaveCount(0);

      const viewer = await withSignedInPage(viewerContext, acceptance.viewer.email);
      await expect(viewer.getByRole("link", { name: "New post" })).toHaveCount(0);
      await expect(viewer.getByRole("button", { name: "Archive selected" })).toHaveCount(0);
      await expect(viewer.getByText("Acceptance Published", { exact: true })).toBeVisible();

      const other = await withSignedInPage(otherContext, acceptance.otherOwner.email);
      await expect(other.getByText("Other Tenant Secret", { exact: true })).toBeVisible();
      await expect(other.getByText("Acceptance Published", { exact: true })).toHaveCount(0);
    } finally {
      await Promise.all([
        ownerContext.close(),
        viewerContext.close(),
