...
function isolateGitEnvironment() {
  // Hooks export Git settings that would otherwise target the caller's worktree.
  const gitEnvironment = new Map(
    git(process.cwd(), "rev-parse", "--local-env-vars")
      .split("\n")
      .map((key) => [key, process.env[key]]),
  );
  for (const key of gitEnvironment.keys()) delete process.env[key];
  return () => {
    for (const [key, value] of gitEnvironment) {
...
test("the push range includes multiple commits and deletions", async () => {
  ...
  const restoreGitEnvironment = isolateGitEnvironment();
  try {
    ...
    git(directory, "config", "user.name", "Image test");
    git(directory, "config", "user.email", "image@example.com");
    await writeFile(join(directory, "Dockerfile"), "FROM scratch\n");
    git(directory, "add", "Dockerfile");
    ...
    git(directory, "commit", "--quiet", "-m", "docs");
    git(directory, "rm", "--quiet", "Dockerfile");
    git(directory, "commit", "--quiet", "-m", "remove image input");
